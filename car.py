...
# --- تمثيل الدركسون البصري التفاعلي (باستخدام components.html لضمان ظهوره) ---
st.subheader("🎮 المقود التفاعلي (Steering Wheel)")

steering_html = f"""
<div style="display: flex; justify-content: center; align-items: center; height: 230px; background-color: transparent;">
...
# عرض الـ HTML بشكل صحيح
components.html(steering_html, height=240)

# --- أدوات التحكم (المنزلقات) ---
...
st.session_state.steering_angle = st.slider(
    ...
    min_value=-90,
    max_value=90,
    value=int(st.session_state.steering_angle),
    step=10
)
...
with col_ctrl2:
    st.session_state.brake = st.slider(
        ...
        min_value=0,
        max_value=100,
        value=int(st.session_state.brake),
        step=10
    )
